...
function MenuItem({ item }: { item: SiteNavigationElement }) {
  ...
  if (item.children === undefined) {
    return (
      <div class="p-4">
        <a class="block hover:underline text-sm uppercase" href={item.url}>
          {item.name}
        </a>
      </div>
    );
  ...
  return (
    !item.children || item.children.length == 0
      ? (
        <a
          href={item.url}
          class="collapse-title uppercase text-sm min-h-[unset] h-auto flex items-center"
        >
          {item.name}
        </a>
      )
      : (
        <div class="collapse collapse-arrow">
          ...
          <div class="collapse-title uppercase text-sm min-h-[unset] h-auto flex items-center">
            {item.name}
          </div>
                ...
                return (
                  <li class="px-4 py-3">
                    <a class="hover:underline" href={node.url}>
                      <span class="text-[13.5px] uppercase font-semibold text-brand">
                        {name}
                      </span>
                    </a>
                      ...
                      {node.children?.map((leaf) => {
                        ...
                        return (
                          <li>
                            <a
                              class={name.search(/tamanho/i) !== -1
                                ? "flex items-center justify-center rounded-full bg-brand  w-10 h-10 text-white"
                                : "hover:underline"}
                              href={leaf.url}
                            >
                              <span
