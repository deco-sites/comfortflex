...
function CartButton({ loading, currency, total, items }: Props) {
  ...
  return (
    <div class="indicator">
      <span
        class={`flex items-center justify-center absolute top-0 right-0 w-4 h-4 bg-white lg:bg-brand  text-black lg:text-white rounded-full text-xs ${
          totalItems === 0 ? "hidden" : ""
        }`}
      >
