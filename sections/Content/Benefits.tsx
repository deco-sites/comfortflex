...
const BenefitItem = ({ benefit }: BenefitItemProps) => {
  return (
    ...
    <div style={`border: ${benefit.benefitLink ? '1px solid #BC81FF' : 'none'}; color:#BC81FF; border-radius:23px;`}
      ...
      <div className="flex-auto flex flex-col gap-1">
        <div className="text-base md:text-lg font-semibold uppercase">{benefit.benefitTitle}</div>
...
const Benefits = ({ benefits }: BenefitsProps) => {
  const id = useId();
  return (
    <div id={id} className={`container grid grid-cols-1 my-5 items-center justify-center gap-3 w-full h-full md:gap-8 md:grid-cols-2 lg:grid-cols-${benefits && benefits.length}`}>
